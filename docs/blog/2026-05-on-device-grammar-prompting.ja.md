...
**0% と 85%。** 同じ 20 タスク。同じ prompt パイプライン。同じ model サイズクラス。この 2 つの数字を分けたのは、HTTP コールの向こう側にいる 1-2 billion パラメータの open-weights LLM が **どれか** だけです。
...
この記事では、その表のすべてのセルを 1 行ずつ解説します。カラムが追加する 4 つの軽い技術。Llama が示す失敗モード (具体的で示唆に富む形)。各構成要素が支払うレイテンシのコスト。すべて再現可能で、コードは [noroshi](https://github.com/velocitylabo/noroshi)、生の bench JSON もリポに入っています。
